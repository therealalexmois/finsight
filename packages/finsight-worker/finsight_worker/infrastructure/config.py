...
from finsight_worker.infrastructure.utils.pyproject import extract_project_field, find_pyproject_path
...
class WorkerSettings(BaseSettings):
    ...
    Attributes:
        name: Имя воркера (из pyproject.toml или значение по умолчанию).
        env: Окружение воркера: local, dev, production.
        host: Имя хоста или IP, где запущен воркер.
        version: Версия приложения (из pyproject.toml или значение по умолчанию).
    """
...
class RedisSettings(BaseSettings):
    """Настройки Redis как брокера и backend Celery.
...
class Settings(BaseSettings):
    """Корневые настройки воркера, объединяющие вложенные секции.

    Attributes:
        worker: Настройки Celery-воркера.
        logging: Настройки логирования.
        redis: Настройки Redis (брокер и backend).
    """
